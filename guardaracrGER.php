...
// Validaciones basicas
	if (!is_numeric($_POST['ced'])){js_redireccion("error.php?msn=Número de Cédula Inválido");exit;}
	if ($_POST['nom'] == '' || $_POST['nom'] == ' '){js_redireccion("error.php?msn=El campo nombre no puede estar en blanco");exit;}
	if (!is_numeric($_POST['cod'])){js_redireccion("error.php?msn=Código de Área Inválido");exit;}
	if (!is_numeric($_POST['tel'])){js_redireccion("error.php?msn=Número de Teléfono Inválido");exit;}
	if ($_POST['cont'] == '0'){js_redireccion("error.php?msn=Debe seleccionar un Número de Contrato");exit;}
	if ($_POST['Cate'] == 0){js_redireccion("error.php?msn=Debe seleccionar un tipo de Acreditación");exit;}
...
// validacion de existencia	
		...
		if (mysql_num_rows($result) != 0){js_redireccion("error.php?msn=La Cédula Número: ".$ced.", ya está registrada");exit;}	
	//exit;
// validacion del tipo de acreditacion	
		...
		if (mysql_num_rows($result) <= 0){js_redireccion("error.php?msn=No le corresponde este tipo de Acreditación");exit;}
		...
		if (mysql_num_rows($result2) >= $cant){
			js_redireccion("error.php?msn=Ha excedido el número de Acreditaciones para el Tipo Seleccionado");exit;}
		$cant = $result["cant"];
		//Codigo de Barras
		$i = strlen($_POST['ced']);
		$campo = $_POST['ced'];
	   while ($i<8)
	   {
		...
		if(strtoupper($extension[$num]) == "JPG") // pregunto si el archivo es JPG
			...
			$strsql = "INSERT INTO acreditado (rif, cod_b, cod_v, cedula, nombre, telf, tipacr, foto, estatus, contrato, fecha_r) VALUES ('$rif','$CB','$CV','$ced','$nom','$tele',$Cate,'$foto',1,'$cont','$hoy')";
			//echo $strsql; exit;
			...
			{
			js_redireccion("error.php?msn=El formato de la imagen es inválido, solo se acepta JPG");
			}
