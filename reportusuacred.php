...
function Header()
{
	...
	$this->Cell(80);
	//Título
	$this->Cell(30,10,'REPORTE DE ACREDITACIONES POR USUARIOS',0,0,'C');
	$this->Ln();
...
function FancyTable($header)
{
	...
	$result= $con->ejecutar("SELECT cedula,nombre,usuario FROM acreditado",$idcon);
	
    while ($fila = mysql_fetch_array($result)) {
	 $this->Cell($w[0],6,$fila[cedula],1,'L');
	 $this->Cell($w[1],6,$fila[nombre],1,'L');
...
$pdf=new PDF();
$header=array('Cédula','Nombre','Usuario');
$pdf->AliasNbPages();
$pdf->AddPage();
$pdf->SetFont('Arial','',8);
